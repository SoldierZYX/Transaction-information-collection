...
| SRC-36KR-001 | https://36kr.com/feed-newsflash | 36Kr 快讯标题、发布时间与链接 | 标题、时间、链接；不保存正文 |
...
4. 公告、政策和新闻仅保存必要元数据、短摘要和链接，不在 MVP 中批量保存全文或解析 PDF。
5. 真实网络访问必须在适配器实现阶段由人工再次确认，并以低频方式运行。
6. 每个网站的抓取与解析代码必须位于独立的来源文件中，具体命名规则见 `docs/source-code-layout.md`。

## 未批准范围

- `SRC-MARKET-001` 日线行情、`SRC-OVERSEAS-001` 海外市场、`SRC-AI-001` AI 仍为待调查状态；未列出的新闻网站仍须单独调查。
